...
public abstract class AbstractHashStruct<K, V> {
    ...
    public V put(K k, V v) {
        ...
        AbstractHashStruct<K, V>.Node toSet = this.search(k, head);
    ...
    private int posOf(K key) {
        ...
        // int hash = this.hasher.hashInt(key.hashCode()).asInt();
        // long convert = UnsafeProvider.normalize(hash);

        // return (int) (convert % (long) this.buckets.length);
        ...
        return now % this.buckets.length;
    }

    private void checkSize() {
        if (this.size == this.resizeThresh) {
            int in = this.resizeThresh *= 2;

            AbstractHashStruct<K, V>.Node[] resize = new AbstractHashStruct.Node[in];
            System.arraycopy(this.buckets, 0, resize, 0, this.buckets.length);
            this.buckets = resize;
        }
    }
    ...
    private AbstractHashStruct<K, V>.Node search(K k, AbstractHashStruct<K, V>.Node head) {
        // Implementation:
        // 4 Step process
        ...
        else return this.create(k, head);
    }

    private AbstractHashStruct<K, V>.Node create(K k, AbstractHashStruct<K, V>.Node previous) {
